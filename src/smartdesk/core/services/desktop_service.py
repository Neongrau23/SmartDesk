...
def update_desktop(old_name: str, new_name: str, new_path: str) -> bool:
        ...
        if old_path_exists:
            try:
                if os.path.exists(new_path):
                    msg = get_text("desktop_handler.warn.target_path_exists", path=new_path)
                    logger.warning(msg)
    ...
    save_desktops(desktops)
    msg = get_text("desktop_handler.success.update", old_name=old_name, new_name=new_name)
    logger.info(msg)
    return True
...
def delete_desktop(name: str, delete_folder: bool = False, skip_confirm: bool = False, parent=None) -> bool:
    ...
    if real_registry_path:
        ...
        if norm_registry == norm_target:
            msg = get_text("desktop_handler.error.delete_critical", path=target_desktop.path)
            logger.error(msg)
    ...
    if delete_folder:
        if os.path.exists(target_desktop.path):
            try:
                ...
                msg = get_text("desktop_handler.success.folder_delete", path=target_desktop.path)
                logger.info(msg)
            except Exception as e:
                msg = get_text("desktop_handler.error.folder_delete", e=e)
                logger.error(msg)
        else:
            msg = get_text("desktop_handler.info.folder_not_found", path=target_desktop.path)
            logger.info(msg)
...
def switch_to_desktop(desktop_name: str, parent=None) -> bool:
    ...
    if not os.path.exists(target_path):
        ...
        if choice_text == get_text("desktop_handler.prompts.path_recreate"):
            ...
            try:
                desktops.remove(target_desktop)
                save_desktops(desktops)
                msg = get_text("desktop_handler.success.removing_config", name=desktop_name)
                logger.info(msg)
    ...
    if active_desktop:
        ...
        logger.info(msg)
        try:
            active_desktop.icon_positionen = get_current_icon_positions(timeout_seconds=3)
    ...
    reg_success = True
    if not update_registry_key(KEY_USER_SHELL, VALUE_NAME, clean_path, winreg.REG_EXPAND_SZ):
        reg_success = False
    if not update_registry_key(KEY_LEGACY_SHELL, VALUE_NAME, clean_path, winreg.REG_SZ):
        reg_success = False
...
def sync_desktop_state_and_apply_icons():
    ...
    msg = get_text("desktop_handler.info.sync_restoring_icons", name=new_active_desktop.name)
    logger.info(msg)
...
def assign_wallpaper(desktop_name: str, source_image_path: str) -> bool:
    ...
    if not os.path.exists(source_image_path):
        msg = get_text("wallpaper_manager.error.source_not_found", path=source_image_path)
        logger.error(msg)
        return False
    ...
    new_path = wallpaper_service.copy_wallpaper_to_datadir(source_image_path, target_desktop.name)
