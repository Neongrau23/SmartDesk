...
class DesktopPage(QWidget):
    ...
    def refresh_linked_programs(self, desktop_name):
        if not hasattr(self, "list_programs"):
            return
    ...
    def setup_connections(self):
        # Sidebar
        if self.list_desktops:
            self.list_desktops.itemSelectionChanged.connect(self.on_selection_changed)
        if self.btn_add:
            self.btn_add.clicked.connect(self.show_create_view)

        # Details Actions
        if self.btn_edit_name:
            self.btn_edit_name.clicked.connect(self.action_rename_desktop)
        ...
        if self.btn_activate:
            self.btn_activate.clicked.connect(self.action_activate_desktop)
        if self.btn_delete:
            self.btn_delete.clicked.connect(self.action_delete_desktop)

        # Create Actions
        if self.btn_browse:
            self.btn_browse.clicked.connect(self.action_browse_folder)
        if self.btn_confirm_create:
            self.btn_confirm_create.clicked.connect(self.action_create_desktop)
        if self.btn_cancel_create:
            self.btn_cancel_create.clicked.connect(lambda: self.stack_content.setCurrentIndex(0))  # Zurück zu Empty oder Last
    ...
    def action_rename_desktop(self):
        ...
        new_name, ok = QInputDialog.getText(self, "Desktop umbenennen", f"Neuer Name für '{old_name}':", QLineEdit.Normal, old_name)
    ...
    def action_change_wallpaper(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getOpenFileName(self, "Wallpaper auswählen", "", "Bilder (*.png *.jpg *.jpeg *.bmp)", options=options)
    ...
    def action_activate_desktop(self):
        if not hasattr(self, "current_desktop"):
            return

        reply = QMessageBox.question(self, "Bestätigung", f"Möchten Sie wirklich zu '{self.current_desktop.name}' wechseln?\nDer Explorer wird neu gestartet.", QMessageBox.Yes | QMessageBox.No)
