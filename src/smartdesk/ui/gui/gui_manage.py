...
import sys
import logging
from PySide6.QtWidgets import QApplication, QWidget, QPushButton, QVBoxLayout, QMessageBox, QFileDialog, QInputDialog, QMenu
from PySide6.QtUiTools import QUiLoader
from PySide6.QtCore import QFile, QIODevice, Qt, Signal, QPropertyAnimation, QEasingCurve, QRect, QTimer, QEvent

# --- Pfad-Hack entfernt: Bitte via 'python -m smartdesk.ui.gui.gui_manage' starten ---
...
try:
    ...
    class FakeService:
        ...
        def update_desktop(self, old, new, path):
            return True

        def assign_wallpaper(self, name, path):
            return True

        def delete_desktop(self, name, delete_folder=False):
            return True
...
class ManageDesktopsWindow(QWidget):
    ...
    def __init__(self):
        ...
        # Signale verbinden
        if self.btn_close:
            self.btn_close.clicked.connect(self.close_panel_animated)
        if self.btn_rename:
            self.btn_rename.clicked.connect(self.action_rename)
        if self.btn_wallpaper:
            self.btn_wallpaper.clicked.connect(self.action_wallpaper)
        if self.btn_more:
            self.btn_more.clicked.connect(self.action_more_options)
    ...
    def _set_actions_enabled(self, enabled):
        ...
        if self.btn_wallpaper:
            self.btn_wallpaper.setEnabled(enabled)
        if self.btn_more:
            self.btn_more.setEnabled(enabled)
