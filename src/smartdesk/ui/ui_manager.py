...
class DesktopCreatorGUI:
    def __init__(self, root):
        ...
        # Titel (Lokalisiert)
        title = tk.Label(main_frame, text=get_text("gui.create.title"), 
                         font=('Segoe UI', 11),
        ...
        name_label = tk.Label(main_frame, text=get_text("gui.create.label_name"),
                              font=('Segoe UI', 9),
        ...
        self.pfad_label = tk.Label(main_frame,
        ...
        self.path_entry = tk.Entry(pfad_container,
                                   font=('Segoe UI', 10),
                                   bg=self.bg_input,
                                   fg=self.fg_primary,
                                   insertbackground=self.fg_primary,
        ...
        browse_btn = tk.Button(pfad_container,
        ...
        create_btn = tk.Button(button_frame,
                               ...
                               pady=6,
                               cursor="hand2",
                               command=self.submit_data) # Geändert
        ...
        cancel_btn = tk.Button(button_frame,
                               ...
                               pady=6,
                               cursor="hand2",
                               command=self.cancel_and_close) # Geändert
    ...
    def browse_folder(self):
        folder = filedialog.askdirectory(title=get_text("gui.create.browse_title")) # Titel lokalisiert
        if folder:
    ...
    def submit_data(self):
        ...
        # --- Nur Validierung ---
        if not name:
            messagebox.showerror(get_text("gui.create.error_title"), 
                                 get_text("gui.create.error_no_name"))
            return
        
        if not path:
            messagebox.showerror(get_text("gui.create.error_title"), 
                                 get_text("gui.create.error_no_path"))
            return
