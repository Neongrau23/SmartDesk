...
try:
    ...
    try:
        ...
        smartdesk_dir = os.path.dirname(current_dir)
        ...
        from smartdesk.handlers import tray_manager
...
class SmartDeskGUI(ctk.CTk):
    ...
    def show_dashboard(self):
        ...
        self.create_stat_card(self.main_frame, 1, 0, "Gesamt Desktops", str(total), "💻")
        ...
        self.status_text = ctk.CTkTextbox(status_frame, height=150, font=ctk.CTkFont(size=12))
    ...
    def update_status_info(self):
        ...
        try:
            ...
            hotkey_status = f"✓ Aktiv (PID: {hotkey_pid})" if hotkey_pid else "○ Inaktiv"
            ...
            # Aktiver Desktop
            desktops = desktop_handler.get_all_desktops()
            active_desktop = next((d.name for d in desktops if d.is_active), "Kein aktiver Desktop")
...
"""
            ...
            self.status_text.insert("0.0", f"Fehler beim Laden der Status-Informationen: {e}")
    ...
    def switch_desktop(self, name):
        ...
        if messagebox.askyesno("Desktop wechseln", f"Möchtest du zu '{name}' wechseln?\n\nDer Explorer wird neu gestartet."):
            try:
                if desktop_handler.switch_to_desktop(name):
                    ...
                    messagebox.showerror("Fehler", "Desktop konnte nicht gewechselt werden.")
            except Exception as e:
                messagebox.showerror("Fehler", f"Fehler beim Wechseln: {e}")
    
    def delete_desktop(self, name):
        """Löscht einen Desktop"""
        desktop = next((d for d in desktop_handler.get_all_desktops() if d.name == name), None)
        if not desktop:
            return
    ...
    def update_create_mode(self):
        ...
        if self.create_mode.get() == "existing":
            ...
            self.create_hint.configure(text="Wähle einen übergeordneten Ordner, in dem der neue Ordner erstellt wird")
    ...
    def assign_wallpaper_action(self):
        ...
        try:
            ...
            messagebox.showinfo("Erfolg", f"Wallpaper wurde '{desktop_name}' zugewiesen!")
            self.wallpaper_path_entry.delete(0, "end")
        except Exception as e:
    ...
    def show_hotkey_log(self):
        ...
        log_text = ctk.CTkTextbox(log_window, font=ctk.CTkFont(family="Courier", size=12))
    ...
    def show_settings(self):
        ...
        settings_actions = [
            ("🔄 Icons speichern", self.save_icons_action, "Speichert die aktuelle Icon-Position"),
            ("🔁 Explorer neustarten", self.restart_explorer_action, "Startet den Windows Explorer neu"),
            ("🔧 Registry wiederherstellen", self.restore_registry_action, "Stellt Registry-Pfade wieder her"),
        ]
    ...
    def restart_explorer_action(self):
        ...
        if messagebox.askyesno("Explorer neustarten", "Möchtest du den Windows Explorer wirklich neu starten?"):
            try:
                system_manager.restart_explorer()
    ...
    def restore_registry_action(self):
        ...
        if messagebox.askyesno("Registry wiederherstellen", "Möchtest du die Registry-Pfade wirklich wiederherstellen?"):
            try:
                import subprocess
                import platform
                ...
                script_path = os.path.abspath(
                    os.path.join(os.path.dirname(__file__), '..', '..', '..', 'scripts', 'restore.bat')
                )
                
                if not os.path.exists(script_path):
                    messagebox.showerror("Fehler", f"Skript nicht gefunden:\n{script_path}")
                    return
                
                subprocess.call(['cmd', '/c', script_path])
                messagebox.showinfo("Erfolg", "Registry-Wiederherstellung abgeschlossen!")
            except Exception as e:
